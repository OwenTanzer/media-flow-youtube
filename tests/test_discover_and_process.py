...
def test_main_exits_without_acting_when_lock_is_held(monkeypatch):
    ...
    monkeypatch.setattr(discover_and_process.job_lock, "acquire_lock", lambda folder_id, ttl_seconds: False)
    ...
    release_calls = []
    monkeypatch.setattr(discover_and_process.job_lock, "release_lock", lambda folder_id: release_calls.append(1))
...
def test_main_runs_discovery_then_batch_and_releases_lock_when_acquired(monkeypatch):
    monkeypatch.setattr(discover_and_process.settings, "drive_folder_id", "folder-id")
    monkeypatch.setattr(discover_and_process.job_lock, "acquire_lock", lambda folder_id, ttl_seconds: True)
    ...
    monkeypatch.setattr(discover_and_process.job_lock, "release_lock", lambda folder_id: order.append("release"))
